...
# -----------------------
update_interval = 300
def update_loop(interval=update_interval):
    while True:
...
def dashboard():
    ...
    """
    return render_template_string(html_template, rows=rows, top10=top10, map_points=map_points)
...
def export_pdf():
    ...
    table_data = [["Indicator","Type","Source","Severity","First Seen","Last Seen","Description"]] + list(rows)
...
def manual_refresh():
    data = fetch_otx_data() + fetch_abuseipdb_data() + fetch_talos_data()
    save_data_to_db(data)
    return f"Data manually refreshed at {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}"
...
def charts_dashboard():
    ...
    conn.close()
    severity_data = {r[0]:r[1] for r in severity_rows}
    type_data = {r[0]:r[1] for r in type_rows}
    severities = ["Low","Medium","High","Critical"]
    ...
    <head>
        ...
        <style>
            ...
            canvas { max-width:800px; margin:30px auto; background:#2b2b3b; padding:20px; border-radius:10px;}
        </style>
    </head>
    <body>
        ...
        <script>
            ...
            new Chart(ctx1,{type:'bar',data:{labels:{{severities|tojson}},datasets:[{label:'Indicators',data:{{severity_counts|tojson}},backgroundColor:['#00ffcc','#ffa500','#ff4c4c','#8b0000'],borderColor:['#00ffcc','#ffa500','#ff4c4c','#8b0000'],borderWidth:1}]},options:{responsive:true,scales:{y:{beginAtZero:true}}}});
            var ctx2 = document.getElementById('typeChart').getContext('2d');
            new Chart(ctx2,{type:'pie',data:{labels:{{types|tojson}},datasets:[{label:'Type Distribution',data:{{type_counts|tojson}},backgroundColor:['#00ffcc','#ffa500','#ff4c4c'],borderWidth:1}]},options:{responsive:true}});
        </script>
    </body>
    </html>
    """
