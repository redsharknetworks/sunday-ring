...
def fetch_otx_data():
    ...
    for _ in range(10):
        ...
        indicator = f"{random.randint(1,255)}.{random.randint(0,255)}.{random.randint(0,255)}.{random.randint(0,255)}" if typ=="IP" else f"malicious{random.randint(1,50)}.com" if typ=="Domain" else os.urandom(8).hex()
        data.append({
            "indicator": indicator,
            "type": typ,
            "source": "OTX",
...
def dashboard():
    ...
    # top10
    cursor.execute('SELECT indicator, type, source, COUNT(*) FROM indicators GROUP BY indicator ORDER BY COUNT(*) DESC LIMIT 10')
...
<html>
<head>
<title>CTI Dashboard v5.1</title>
<meta charset="utf-8" />
...
canvas{background-color:#2b2b3b; border-radius:10px; margin:20px auto; display:block;}
</style>
</head>
<body>
<h1 style="text-align:center;">CTI Dashboard v5.1</h1>
...
{% for r in rows %}<tr>
<td>{{ r[1] }}</td><td>{{ r[2] }}</td><td>{{ r[3] }}</td><td class="{{ r[4]|lower }}">{{ r[4] }}</td><td>{{ r[5] }}</td><td>{{ r[6] }}</td></tr>{% endfor %}
</table>
...
{% for t in top10 %}<tr>
<td>{{ t[0] }}</td><td>{{ t[1] }}</td><td>{{ t[2] }}</td><td>{{ t[3] }}</td></tr>{% endfor %}
</table>
...
var severity_counts=[{% for s in severities %}{{ rows|selectattr("4","equalto",s)|list|length }}{% if not loop.last %},{% endif %}{% endfor %}];
var types=["IP","Domain","Hash"];
var type_counts=[{% for t in types %}{{ rows|selectattr("2","equalto",t)|list|length }}{% if not loop.last %},{% endif %}{% endfor %}];

new Chart(document.getElementById('severityChart').getContext('2d'),{
 type:'bar', data:{labels:severities,datasets:[{label:'Number of Indicators',data:severity_counts,backgroundColor:['#00ffcc','#ffa500','#ff4c4c','#8b0000'],borderColor:['#00ffcc','#ffa500','#ff4c4c','#8b0000'],borderWidth:1}]}, options:{responsive:true,scales:{y:{beginAtZero:true}}}
});
new Chart(document.getElementById('typeChart').getContext('2d'),{
 type:'pie', data:{labels:types,datasets:[{label:'Type Distribution',data:type_counts,backgroundColor:['#00ffcc','#ffa500','#ff4c4c'],borderWidth:1}]}, options:{responsive:true}
});
</script>
</body>
</html>
"""
    return render_template_string(html_template, rows=rows, top10=top10_rows, map_points=map_points)
...
def export_pdf():
    ...
    elements.append(Paragraph("CTI Dashboard Export",styles['Heading1']))
    elements.append(Spacer(1,12))
    table_data=[["Indicator","Type","Source","Severity","First Seen","Last Seen","Description"]]+[list(r) for r in rows]
    table=Table(table_data,repeatRows=1)
    table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.darkblue),('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),('ALIGN',(0,0),(-1,-1),'LEFT'),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('BOTTOMPADDING',(0,0),(-1,0),12),('BACKGROUND',(0,1),(-1,-1),colors.HexColor('#2b2b3b')),('GRID',(0,0),(-1,-1),0.5,colors.grey)]))
