...
def apply_retention_policy():
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("DELETE FROM threats WHERE created_at < %s", (cutoff,))
    conn.commit()
    conn.close()
...
def insert_dummy_data():
    ...
    for i in range(20):
        ...
        """, (pulse, indicator, "domain", "Medium", "OTX", score, datetime.utcnow()))
    conn.commit()
    conn.close()
...
def fetch_otx_data():
    ...
    for pulse in pulses[:10]:
        ...
        for ind in pulse.get("indicators", []):
            ...
            INSERT INTO threats (pulse, indicator, type, classification, mitre, risk_score, created_at)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
            """, (name, val, typ, "Medium", "OTX", score, datetime.utcnow()))
    conn.commit()
    conn.close()
...
def dashboard():
    ...
    c.execute("SELECT * FROM threats ORDER BY created_at DESC LIMIT 50")
    rows = c.fetchall()
    conn.close()
    return render_template_string(TEMPLATE, table_data=rows)
...
def pdf_report():
    ...
    for r in rows:
        table_data.append([r["type"], r["indicator"], r["count"], round(r["avg_risk"], 1)])
    ...
    return send_file(buffer, as_attachment=True,
                     download_name=f"RedShark_report_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.pdf",
...
def json_report():
    ...
    output = io.BytesIO()
    output.write(json.dumps(rows, indent=2, default=str).encode())
    output.seek(0)
