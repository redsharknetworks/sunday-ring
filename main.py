import os, sqlite3, requests, json, csv, io, random, threading, time
...
def malaysia_map():
    ...
    sizes = [18 if s>=85 else 14 if s>=70 else 10 for s in sev]
    fig = go.Figure()
...
<ul>{% for b in highlights %}<li class="{% if 'Severity 85' in b.statement or 'Severity 90' in b.statement %}blink{% endif %}">{{b.statement}}</li>{% endfor %}</ul></div>
