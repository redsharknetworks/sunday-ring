...
def threat_score(sev):
    return {"Low": 20, "Medium": 50, "High": 75, "Critical": 95}[sev]

# ---------------- FETCH REAL FEEDS ---------------- #
def fetch_otx_iocs():
    headers = {"X-OTX-API-KEY": OTX_KEY}
    url = "https://otx.alienvault.com/api/v1/indicators/export?limit=50&type=IPv4"
    try:
...
def fetch_abuseipdb():
    ...
    url = "https://api.abuseipdb.com/api/v2/blacklist?confidenceMinimum=80&limit=50"
    try:
        r = requests.get(url, headers=headers, timeout=10)
        iocs = []
        if r.status_code == 200:
...
def export_ids():
    ...
    mem=io.BytesIO(rules.encode())
    # Create zip containing the IDS rules
    zip_mem=io.BytesIO()
    with zipfile.ZipFile(zip_mem,'w',zipfile.ZIP_DEFLATED) as z:
        z.writestr("redshark.rules",mem.getvalue())
    zip_mem.seek(0)
    return send_file(zip_mem, as_attachment=True, download_name="redshark_ids.zip")
