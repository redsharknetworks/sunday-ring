...
def initial_dummy_load():
    """Fallback dummy from recent real IOCs"""
    events=[]
    otx_events = fetch_otx()
    abuse_events = fetch_abuseipdb()
    combined = otx_events + abuse_events
    fallback = combined[:10]  # pick first 10
    for e in fallback:
        lat, lon = map_to_ibu_negeri()[1:3]
        events.append({
            "ip": e["ip"], "source": e["source"], "severity": e["severity"],
...
@app.route("/")
def dashboard():
    return render_template_string("""[KEEP HTML TEMPLATE FROM V22.5]""")
