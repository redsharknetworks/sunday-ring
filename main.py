...
def generate_charts_bg():
    ...
    while True:
        ...
        time.sleep(600) # regenerate every 10 minutes

def start_background_charts():
...
def dashboard():
    return render_template_string(TEMPLATE, 
                                  ...
                                  type_chart=charts.get("type_chart"),
                                  top10_chart=charts.get("top10"),
                                  heatmap=generate_heatmap())

# ---------------- REPORTS (LIMITED TO 50 ROWS) ----------------
...
def json_report():
    ...
    hashes = c.execute("SELECT id, pulse, hash as indicator, 'hash' as type, classification, mitre, risk_score, created_at FROM threat_hashes ORDER BY created_at DESC LIMIT 50").fetchall()
    conn.close()
    return jsonify([dict(x) for x in threats+hashes])
...
def pdf_report():
    ...
    for r in threats+hashes:
        ...
        data.append([r["id"], pulse, indicator, r["type"], r["classification"], r["mitre"], r["risk_score"], r["created_at"]])
