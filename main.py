...
def fetch_feeds():
    try:
        # Demo IPs and URLs
        demo = [("1.1.1.1","ip",85),("8.8.8.8","ip",90),("malicious.com","domain",70)]
        for i,typ,sev in demo: insert_threat(i,typ,sev)
    except: pass

# ---------------- SCHEDULER ----------------
def scheduler(): fetch_feeds(); threading.Timer(900,scheduler).start()
...
def malaysia_map():
    ...
    fig.add_trace(go.Scattermapbox(lat=lat,lon=lon,mode="markers",
                                   ...
                                   text=[f"Severity: {s}" for s in sev], hoverinfo="text"))
...
<style>body{background:#0b1b2a;color:#A3B8CC;font-family:Arial;}
...
<div class="card"><h3>CTI Highlight at {{highlight_time}}</h3><ul>{% for b in highlights %}<li>{{b.statement}}</li>{% endfor %}</ul></div>
...
<div class="card"><h3>Latest Threat Indicators</h3><table id="threat_table"><thead><tr><th>ID</th><th>Indicator</th><th>Type</th><th>Sector</th><th>Severity</th><th>MITRE</th><th>Timestamp</th></tr></thead><tbody>{% for r in rows %}<tr><td>{{r.id}}</td><td>{{r.indicator}}</td><td>{{r.type}}</td><td>{{r.sector}}</td><td>{{r.severity}}</td><td>{{r.mitre}}</td><td>{{r.created}}</td></tr>{% endfor %}</tbody></table></div>
<div class="center">
