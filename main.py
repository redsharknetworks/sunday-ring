...
def fetch_otx_data():
    ...
    sectors = ["Finance","Healthcare","Energy","Gov","Tech"]
    for _ in range(15):
...
def save_data(records):
    ...
    for r in records:
        cursor.execute('''
            INSERT INTO indicators (indicator, type, source, severity, sector, first_seen, last_seen, description)
            VALUES (?,?,?,?,?,?,?,?)
        ''', (r["indicator"], r["type"], r["source"], r["severity"], r["sector"], r["first_seen"], r["last_seen"], r["description"]))
    conn.commit()
    conn.close()
...
def dashboard():
    ...
    # Prepare map points with Malaysia lat/lon approximation
    map_points = []
    for r in rows:
    ...
    # Compute top 10 indicators by frequency
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute('''
    ...
    <html>
    <head>
        <title>RedShark Threat Intelligence Dashboard v4</title>
        <meta charset="utf-8" />
    ...
    <body>
        <h1 style="text-align:center;">RedShark Threat Intelligence Dashboard</h1>
...
def export_pdf():
    ...
    elements.append(Paragraph("RedShark Threat Intelligence Dashboard Export", title_style))
