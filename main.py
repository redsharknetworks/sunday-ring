...
def fetch_abuseipdb_data(ip_list):
    ...
    for ip in ip_list:
        try:
            response = requests.get(ABUSEIPDB_URL, headers=headers, params={"ipAddress": ip, "maxAgeInDays": 90})
            response.raise_for_status()
            data = response.json().get("data", {})
            if data.get("abuseConfidenceScore", 0) > 50:
                insert_event(ip, "AbuseIPDB", "High", f"Abuse confidence score: {data.get('abuseConfidenceScore')}")
        except Exception as e:
            print(f"[AbuseIPDB] Error fetching data for {ip}: {e}")
