...
def fetch_otx_data():
    ...
    sectors = ["Finance","Healthcare","Energy","Government","Retail","Tech"]
    data = []
    for _ in range(10):
...
def save_data_to_db(records):
    ...
    for r in records:
        cursor.execute('''
            INSERT INTO indicators (indicator, type, sector, source, severity, first_seen, last_seen, description)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (r["indicator"], r["type"], r["sector"], r["source"], r["severity"], r["first_seen"], r["last_seen"], r["description"]))
    conn.commit()
    conn.close()
