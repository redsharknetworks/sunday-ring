...
def dashboard():
    ...
    severity_count = {s:0 for s in SEVERITIES}
    for e in EVENTS:
...
<div id="map"></div>
<table>
<tr><th>Timestamp</th><th>Asset</th><th>Event Type</th><th>Severity</th><th>Source IP</th><th>OTX Pulses</th><th>AbuseIPDB</th></tr>
{% for e in events %}
<tr>
<td>{{ e.timestamp }}</td>
<td>{{ e.asset }}</td>
...
// Leaflet Malaysia Heatmap
var map = L.map('map').setView([4.2105,101.9758],6);
L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18}).addTo(map);
var events = {{ events|tojson }};
var city_coords = {{ city_coords|tojson }};
events.forEach(e=>{
    var coord = city_coords.find(c=>c.city===e.city);
    if(coord){
        L.circle([coord.lat, coord.lng],{ color:'red', fillColor:'#f03', fillOpacity:0.5, radius:5000 }).addTo(map)
        .bindPopup(`${e.asset}<br>${e.type}<br>${e.severity}`);
    }
});
</script>
</body>
